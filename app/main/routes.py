...
def blog():
    ...
    next_url = (
        url_for("main.blog", page=all_posts.next_num) if all_posts.has_next else None
    )

    # get the previous page url
    prev_url = (
        url_for("main.blog", page=all_posts.prev_num) if all_posts.has_prev else None
    )
...
def tag(tag):
    ...
    return render_template("main/tag_articles.html", title=title, tag=tag, posts=posts)
...
def contribute():
    ...
    return render_template(
        "main/md_pages.html", title="Contribute", md_render=contribute
    )
...
def about():
    ...
    return render_template("main/md_pages.html", title="About", md_render=about)
...
def author(username):
    ...
    my_posts = Post.query.filter_by(user_id=user.id).order_by(Post.timestamp.desc())
...
def search():
    ...
    return render_template("main/search.html", title="Search", posts=posts, total=total)
...
def acme1():
    return send_from_directory(
        ...
        filename="TRcOHURRJJEDaaas6wgHsP232DQaqY0HnHvadMU-SMY",
    )
...
def acme2():
    return send_from_directory(
        "static/.well-known/acme-challenge",
        filename="zO6V6tGs5eWBqa77cRt_oaS9osezokFNHd_YNThG5sU",
    )
